...
double Child_swap_mcmc_move::propose_move(long time, particle::Particle& part, smc::rng* rng) const
{
    // Ignore leaf nodes
    if(part->node->is_leaf()) return 1.0;
...
std::string Child_swap_mcmc_move::get_name() const
{
    return "Child_swap_mcmc_move";
}
