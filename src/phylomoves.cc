...
double log_likelihood(OnlineCalculator calc, long time, const particle& X)
{
    // XXX: FIX
...
int uniform_bl_mcmc_move::do_move(long time, smc::particle<particle>& from, smc::rng* rng) const {
    ...
    std::shared_ptr< phylo_node > new_node = std::make_shared< phylo_node >();
    new_node->child1 = cur_node->child1;
    new_node->child2 = cur_node->child2;
    new_node->id = calc->get_id();

    double cur_ll = log_likelihood(*calc, time, *part);
    ...
    double alpha = exp(log_likelihood(*calc, time, *part) - cur_ll);
...
int rooted_merge::do_move(long time, smc::particle<particle>& p_from, smc::rng* rng) const
{
    ...
    part->pp = pp;
    std::vector< std::shared_ptr< phylo_node > > prop_vector = uncoalesced_nodes(pp);
    ...
    p_from.AddToLogWeight(logLikelihood(time, *part) - log(d_prob));
